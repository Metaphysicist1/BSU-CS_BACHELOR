...
void ComputeSeidel(double **a,int maxIter,double * start_values,int n,double * B){
    ...
    do                            //Perform iterations to calculate x1,x2,...xn
    {
        ...
        for (int i=0;i<n;i++)                //Loop that calculates x1,x2,...xn
        {
            ...
            if (error<=EPSILON)            //Compare the ne value with the last value
                flag++;
...
void FinalSeidel(double **A,int maxIter,double * start_values,int n,double * B){
   
   
   double *FAnswer=new double[n];
   double *LAnswer=new double[n];
	int iter=0;
   for(int i=0;i<n;i++){
   	    FAnswer[i]=0;
   	    LAnswer[i]=0;
   ...
   double error=0;
	...
	for(int i=0;i<n;i++){
		...
		if(error<fabs(LAnswer[i]-FAnswer[i]))error=fabs(LAnswer[i]-FAnswer[i]);
		FAnswer[i]=LAnswer[i];
		cout<<"x["<<i+1<<"]="<<LAnswer[i]<<"    ";
	}   
	cout<<" norma - >";
	cout<<error<<endl;
   	iter++;
...
//void ComputeValues(double **a,int maxIter,double * start_values,int n,double * B){
//	
//	int flag=0;
...
int main(void){
	...
	cout<<"------------------- Check apllicability of matrix ------------------------"<<endl;
	
//	if(isMethodApllied(n,A)){
	cout<<"Before computation"<<endl;
	Print_roots(start_values,n);
	//ComputeValues(A,maxIter,start_values,n,B);
	cout<<"After computation"<<endl;
	
	FinalSeidel(A,maxIter,start_values,n,B);
	Print_roots(B,n);
//	}
	return 0;
}
